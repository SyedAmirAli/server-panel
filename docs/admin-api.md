...
const toggleKey = (id: string, isActive?: boolean) =>
  api<ApiKeyView>(`/admin/keys/${id}/toggle-active`, { method: "PATCH", body: JSON.stringify(isActive === undefined ? {} : { isActive }) });
const refreshKey = (id: string) =>
  api<ApiKeySecretView>(`/admin/keys/${id}/refresh`, { method: "POST" });
const deleteKey = (id: string) =>
  api<ApiKeyView>(`/admin/keys/${id}`, { method: "DELETE" });
```
...
// response 201 — note: no password field
{
  ...
  "data": {
    "id": "clx9abc123", "name": "Mailcow SMTP", "host": "mail.appszonebd.com",
    "port": 465, "username": "noreply@appszonebd.com",
    "tls": { "rejectUnauthorized": false }, "requireTLS": false, "secure": true,
    "createdAt": "2026-06-24T10:00:00.000Z", "updatedAt": "2026-06-24T10:00:00.000Z"
  }
}
```
...
interface Mailbox {
  id: string;
  address: string;
  imapHost: string; imapUser: string;
  smtpHost: string; smtpUser: string;
